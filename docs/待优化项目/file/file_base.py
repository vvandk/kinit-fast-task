...
class FileBase:
    ...
    @classmethod
    async def async_generate_temp_file_path(cls, filename: str = None, suffix: str = None) -> str:
        """
        生成临时文件路径，生成规则：
        1. filename 参数或者 suffix 参数必须填写一个
        2. filename 参数和 suffix 参数都存在则优先取 suffix 参数为后缀
        ...
        if not filename and not suffix:
            ...
            suffix = os.path.splitext(filename)[-1]
        return f"{await cls.async_generate_temp_dir_path()}/{cls.get_random_filename(suffix)}"
    ...
    async def async_generate_temp_dir_path(cls) -> str:
        ...
        file_dir = AsyncPath(settings.system.TEMP_DIR) / date
