...
class ExcelManage:
    ...
    def open_sheet(
        self, sheet_name: str = None, file: str = None, read_only: bool = False, data_only: bool = False
    ) -> None:
        """
        初始化 excel 文件
    ...
    async def save_excel(self, path: str = "excel_manage"):
        """
        ...
        await AsyncPath(file_path).parent.mkdir(parents=True, exist_ok=True)
    ...
    def __set_row_style(
        ...
        row: int,
        max_column: int,
        alignment_style: AlignmentModel = None,
        font_style: FontModel = None,
        pattern_fill_style: PatternFillModel = None,
    ):
        """
        设置行样式
        :param row: 行
        ...
        :return:
        """
        if not alignment_style:
            alignment_style = AlignmentModel()
        if not font_style:
            font_style = FontModel()
        if not pattern_fill_style:
            pattern_fill_style = PatternFillModel()
        ...
        for row in self.sheet.rows:
            for cell in row:
                if cell.value:
                    ...
                    # 在对比单元格文本时需要将中文字符识别为1.7个长度，英文字符识别为1个，这里只需要将文本长度直接加上中文字符数量即可  # noqa E501
