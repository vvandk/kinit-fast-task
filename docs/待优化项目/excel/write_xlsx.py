...
class WriteXlsx:
    ...
    async def create_excel(self, file_path: str = None, sheet_name: str = "sheet1", save_static: bool = False) -> None:
        """
        创建 excel 文件
        ...
        if not file_path:
            if save_static:
                ...
                self.file_path = await FileBase.async_generate_temp_file_path(suffix="xlsx")
        elif not os.path.isabs(file_path):
            if save_static:
                self.file_path = FileBase.generate_static_file_path(path="write_xlsx", filename=file_path)
            else:
                self.file_path = await FileBase.async_generate_temp_file_path(filename=file_path)
        else:
            self.file_path = file_path
        await AsyncPath(self.file_path).parent.mkdir(parents=True, exist_ok=True)
