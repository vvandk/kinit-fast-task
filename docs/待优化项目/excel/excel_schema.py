...
class AlignmentModel(BaseModel):
    horizontal: str = Field(
        "center", description="水平对齐方式。可选值：'left'、'center'、'right'、'justify'、'distributed'"
    )
    vertical: str = Field(
        "center", description="垂直对齐方式。可选值：'top'、'center'、'bottom'、'justify'、'distributed'"
    )
    textRotation: int = Field(0, description="文本旋转角度（以度为单位）。默认为 0。")
    wrapText: bool = Field(
        None, description="自动换行文本。设置为 True 时，如果文本内容超出单元格宽度，会自动换行显示。"
    )
    shrinkToFit: bool = Field(
        None, description="缩小字体以适应单元格。设置为 True 时，如果文本内容超出单元格宽度，会自动缩小字体大小以适应。"
    )
    indent: int = Field(0, description="文本缩进级别。默认为 0。")
    relativeIndent: int = Field(0, description="相对缩进级别。默认为 0。")
    justifyLastLine: bool = Field(
        None, description="对齐换行文本的末尾行。设置为 True 时，如果设置了文本换行，末尾的行也会被对齐。"
    )
...
class FontModel(BaseModel):
    ...
    underline: str = Field(
        None, description="下划线样式。可选值：'single'、'double'、'singleAccounting'、'doubleAccounting'"
    )
